...
export function Construction() {
  ...
  const precisionPills = tArray('construction.precision.pills');
  const insightCards = tRaw<{ title: string; img: string }[]>('construction.insights.cards') ?? [];
  const deliverLinks = tRaw<{ label: string }[]>('construction.whatWeDeliver.links') ?? [];
  ...
  const bgImageWidth   = useTransform(cardScroll, [0, 1], ['50%', '100%']);
  ...
  // Unsplash images per "What We Deliver" link — revealed on hover
  const whatWeDeliverImages = [
    '1503387762-592deb58ef4e',    // Design — architectural blueprints
    '1581094794329-c8112a89af12', // Engineering — structural engineering
    '1517089258673-4b72e709a9c5', // Construction — active construction site
  ];
  ...
  return (
    <PageTransition>
      ...
      >
        ...
        <div className="absolute inset-y-0 right-0 w-full lg:w-[75%] pointer-events-none">
          ...
          <div
            ...
            style={{ height: '40%', background: 'linear-gradient(to bottom, var(--bg-dark-section) 0%, transparent 100%)' }}
          />
          {/* Bottom feather */}
          <div
            className="absolute inset-x-0 bottom-0 pointer-events-none"
            style={{ height: '40%', background: 'linear-gradient(to top, var(--bg-dark-section) 0%, transparent 100%)' }}
          />
          {/* Right feather */}
          <div
            className="absolute inset-y-0 right-0 pointer-events-none"
            style={{ width: '12%', background: 'linear-gradient(to left, var(--bg-dark-section) 0%, transparent 100%)' }}
          />
          {/* Brand tint */}
          <div
      ...
      >
        ...
        {whatWeDeliverImages.map((imgId, i) => (
          <div
            key={i}
            className="absolute inset-0 pointer-events-none"
            style={{ opacity: hoveredLink === i ? 1 : 0, transition: 'opacity 0.6s ease' }}
          >
            <img
            ...
            />
            {/* Radial center softener */}
            <div
              className="absolute inset-0"
              style={{ background: 'radial-gradient(ellipse 60% 50% at 50% 50%, transparent 10%, var(--bg-secondary) 85%)' }}
            />
          </div>
        ))}
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 text-center">
          ...
          >
            ...
            >
              {t('construction.whatWeDeliver.label')}
            </p>
              ...
              {deliverLinks.map((link, i) => (
                ...
                >
                  <Link
                    ...
                    style={{ border: '1.5px solid var(--border-color)', fontFamily: 'var(--font-ui)' }}
                  >
                    {link.label}
                    <span className="text-[#AEE37B] text-xs leading-none">↗</span>
                  </Link>
                </motion.div>
              ))}
            </div>
            ...
            >
              {t('construction.whatWeDeliver.subtitle')}
            </p>
          </motion.div>
        </div>
      </section>

      {/* ── Section 3: Process Cards (scroll-driven) ─────────────────────── */}
      <div ref={cardSectionRef} className="lg:h-[400vh]">
        ...
        >
          ...
          >
            <motion.img
              src="https://images.unsplash.com/photo-1429497419816-9ca5cfb4571a?w=900&auto=format&fit=crop&q=75"
              alt=""
              aria-hidden="true"
              loading="lazy"
          ...
          <div className="relative z-10 max-w-7xl mx-auto px-6 w-full lg:-mt-12">
              ...
              {[0, 1, 2].map((i) => (
                <div key={i} className="overflow-hidden" style={{ backgroundColor: 'var(--bg-primary)' }}>
                  ...
                  >
                    ...
                    <h3
                      className="text-base font-semibold tracking-tight text-[var(--text-primary)] mb-3"
                      style={{ fontFamily: 'var(--font-ui)' }}
                    >
                      {t(`construction.process.items.${i}.title`)}
                    </h3>
                    <p className="text-[15px] text-[var(--text-secondary)] leading-relaxed" style={{ fontFamily: 'var(--font-body)' }}>
                      {t(`construction.process.items.${i}.desc`)}
                    </p>
                  </motion.div>
                </div>
              ))}
            ...
            <Button onClick={openModal} size="lg" variant="outline">
              {t('construction.process.cta')}
            </Button>
          </div>

          {/* Gradient Orb arrow buttons — grouped center-right, desktop only (lg+) */}
          <div
            className="hidden lg:flex flex-col gap-3 absolute right-6 z-50"
      ...
      {/* ── Section 4: Built with Precision ──────────────────────────────── */}
      <section
      ...
      >
        ...
        <div className="absolute inset-y-0 left-0 w-full lg:w-[70%] pointer-events-none">
          <img
            src="https://images.unsplash.com/photo-1768223927969-f08af51cf730?w=1200&auto=format&fit=crop&q=80"
            alt=""
            aria-hidden="true"
            loading="lazy"
          ...
          <div
            ...
            style={{ width: '12%', background: 'linear-gradient(to right, var(--bg-dark-section) 0%, transparent 100%)' }}
          />
          {/* Accent tint */}
          <div
            className="absolute inset-0 pointer-events-none"
              ...
              <div className="flex flex-col gap-3">
                {precisionPills.map((pill, i) => (
                  ...
                  >
                    ...
                    <span className="text-sm font-medium text-white/85" style={{ fontFamily: 'var(--font-ui)' }}>
                      {pill}
                    </span>
                  </motion.div>
                ))}
      ...
      {/* ── Section 5: Construction Insights ─────────────────────────────── */}
              ...
              >
                ...
                <div className="relative h-48 overflow-hidden">
                  ...
                  {theme !== 'light' && (
                    <div
                      className="absolute inset-x-0 bottom-0 pointer-events-none"
                      style={{ height: '50%', background: 'linear-gradient(to top, var(--bg-primary) 0%, transparent 100%)' }}
                    />
                  )}
                </div>
      ...
      >
        ...
        <div className="absolute inset-0 pointer-events-none">
          <img
            src="https://images.unsplash.com/photo-1510798831971-3eb4b79af851?w=1400&auto=format&fit=crop&q=80"
            alt=""
            aria-hidden="true"
            loading="lazy"
            className="absolute inset-0 w-full h-full object-cover object-center"
          ...
          <div
            className="absolute inset-0"
            style={{ background: 'radial-gradient(ellipse 55% 55% at 50% 50%, transparent 0%, var(--bg-dark-section) 85%)' }}
          />
        </div>
